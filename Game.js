...
class PtolemyIntenseDiatonic extends Tuning
{
    constructor (fundamental)
    {
	super ()
	// A0 is 440 with fundamental 11hz
	this.fundamental = fundamental || 11
    }

    getFrequency (pitchclass, register)
    {
	let n = PtolemyIntenseDiatonic.pitches[pitch]
	let basefreq = PtolemyIntenseDiatonic.frequencies[n]
	return this.fundamental * basefreq * Math.pow(2, register)
    }
}
PtolemyIntenseDiatonic.pitches = {C:0, D:1, E:2, F:3, G:4, A:5, B:6}
PtolemyIntenseDiatonic.frequencies = [24, 27, 30, 32, 36, 40, 45]
Tuning.PtolemyIntenseDiatonic = new PtolemyIntenseDiatonic
...
class Instrument
{
    ...
    {
	this.tuning = Tuning.PtolemyIntenseDiatonic
	this.register = 4
	this.measure = 2
	this.division = 4
	this.time = 0
    ...
    get beat ()
    {
	return this.measure / this.division
    }
    
    note (pitchclass, register)
    {
	register = register === undefined ? this.register : register
	this.register = register	
	let frequency = this.tuning.getFrequency(pitchclass, register)
	this.instrument.tone.frequency.setValueAtTime(frequency, this.time)
    }
...
class MusicControlState
{
    ...
    clone ()
    {
	return Object.assign(new MusicState, this)
    }
}
...
class Musician
{
    constructor ()
    {
	this.controlState = new MusicControlState
    }
